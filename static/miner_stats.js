...
function updateWorkerStats() {
	...
	for (var w in statData.workers) { i++;
		...
		$("#statsPaid"+htmlSafeWorkerName).text(Math.round(statData.workers[w].paid * 100) / 100);
		$("#statsBalance"+htmlSafeWorkerName).text(statData.workers[w].balance);
		$("#statsShares"+htmlSafeWorkerName).text(Math.round(statData.workers[w].shares * 100) / 100);
...
function addWorkerToDisplay(name, htmlSafeName, workerObj) {
	var htmlToAdd = "";
	htmlToAdd = '<div class="boxStats" id="boxStatsRight" style="margin: auto; width: 90%;"><div class="boxStatsList">';
	...
	htmlToAdd+='<div><i class="fa fa-cog"></i> <small>Shares:</small> <span id="statsShares'+htmlSafeName+'">'+(Math.round(workerObj.shares * 100) / 100)+'</span></div>';
